...
class Node:
    def __init__(self, game: Reversi, parent):
        self.game = game
        self.player = game.other # gracz który zrobił ostatni ruch
        self.wins = 0
        self.games = 0
        self.parent = parent
        self.expanded = False
    ...
    def select(self):
        ...
        while selected.expanded:
            if all(n.expanded for n in selected.children.values()):
                selected = max(selected.children.values(), key=lambda n: n.get_UCB(self.root))
            else:
...
if __name__ == '__main__':
    timeit('START')
    play_against_random(10, 100)
    # play_against_alphabeta(10, 400)
    timeit('SHOW')