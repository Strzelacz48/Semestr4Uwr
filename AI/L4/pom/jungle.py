...
class Jungle: # Capitals on top, capitals' cave is '%'
    ...
    def can_move(self, predator, direction):
        ...
        if predator in 'rR' and field == '~' and nfield == '.' and prey != '/': # wychodzenie z wody
            return False
        if predator in 'rR' and nfield == '~': # szczur do wody
            return True
    ...
    def winner(self, verbose=False):
        ...
        if self.last_capture >= 30:
            ...
            if bot > top:
                if verbose: print('bot won by staying with better animals')
                return 'bot'
            elif top > bot:
                if verbose: print('top won by staying with better animals')
                return 'top'
            else:
                if verbose: print('top won by staying with same animals')
                return 'top'
        return None
